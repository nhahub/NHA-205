...
if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
        ...
        if (emailInput) emailInput.classList.remove('error', 'valid');
        if (passwordInput) passwordInput.classList.remove('error', 'valid');
        ...
        if (!emailValidation.valid) {
            if (emailInput && emailError) showError(emailInput, emailError, emailValidation.error);
            isFormValid = false;
        } else {
        ...
        if (!passwordValidation.valid) {
            if (passwordInput && passwordError) showError(passwordInput, passwordError, passwordValidation.error);
            isFormValid = false;
        } else {
            if (passwordInput && passwordError) clearError(passwordInput, passwordError);
        }
        ...
        if (isFormValid) {
            ...
            const formData = {
                ...
                rememberMe: document.querySelector('.checkbox')?.checked || false
            };
            ...
            if (!emailValidation.valid && emailInput) {
                emailInput.focus();
            } else if (!passwordValidation.valid && passwordInput) {
                passwordInput.focus();
            }
        }
    });
...
if (forgotPasswordLink) {
    forgotPasswordLink.addEventListener('click', (e) => {
        ...
        if (!email) {
            ...
            if (emailInput && emailError) showError(emailInput, emailError, 'Please enter your email address first');
            return;
        }
